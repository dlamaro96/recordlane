...
class PaginatedHttpConnector:
    def __init__(self, base_url: str, allowed_hosts: set[str], timeout: float = 20.0, max_pages: int = 10_000, allow_private_networks: bool = False):
        ...
        if parsed.scheme != "https" and parsed.hostname not in {"127.0.0.1", "localhost"} and not allow_private_networks:
        ...
        self._validate_resolution(parsed.hostname, parsed.port or (443 if parsed.scheme == "https" else 80))
    ...
    def _validate_resolution(self, hostname: str, port: int) -> None:
        ...
        for value in addresses:
            ...
            if address in ipaddress.ip_network("169.254.169.254/32") or address in ipaddress.ip_network("fd00:ec2::254/128"):
                raise ValueError("cloud metadata destinations are always forbidden")
            unsafe = address.is_loopback or address.is_link_local or address.is_private or address.is_reserved or address.is_multicast
            if unsafe and not self.allow_private_networks:
                raise ValueError("private connector destination requires explicit administrator authorization")

    def _validate_url(self, url: str) -> None:
        parsed = urlparse(url)
        if parsed.scheme not in {"http", "https"} or not parsed.hostname or parsed.hostname not in self.allowed_hosts or parsed.username or parsed.password:
    ...
    def read(self, path: str, cursor: str | int | None = None, client: httpx.Client | None = None):
        ...
        try:
            for _ in range(self.max_pages):
                ...
                page = HttpPage(payload["items"], payload.get("next_cursor"), bool(payload.get("snapshot_complete", payload.get("next_cursor") is None)))
                yield page
