...
find "$release_root/dist" -mindepth 1 -maxdepth 1 -type f \( \
  -name "recordlane-${release_tag#v}.tar.gz" -o \
  -name "recordlane-${release_tag#v}.tgz" -o \
  -name "recordlane-${release_tag#v}.sbom.spdx.json" -o \
  -name SOURCE_COMMIT -o -name SHA256SUMS \
\) -delete
