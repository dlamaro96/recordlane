...
OPERATOR = {
    ...
    "X-Recordlane-User": "lifecycle.operator",
}
    ...
    with TestClient(app) as client:
        ...
        with SessionLocal() as db:
            source_object = db.scalar(
                select(SourceObject).where(SourceObject.local_id == "stable-42")
            )
            ...
            [
                {
                    ...
                    "sequence": 11,
                    "values": {
                        "name": "Completely renamed supplier",
                        "country": "AE",
        ...
        with SessionLocal() as db:
            ...
            assert (
                db.scalar(
                    select(MembershipHistory).where(
...
def test_cannot_link_is_bound_to_stable_source_objects_across_refresh():
    with TestClient(app) as client:
        ...
        task = next(
            row
            for row in tasks
            if row["kind"] == "duplicate_match"
            and row["payload"].get("left_source_object_id")
        )
        decision = client.post(
            ...
            json={
                ...
                "reason": "Verified as distinct legal entities",
            },
        )
            ...
            [
                {
                    ...
                    "sequence": 2,
                    "values": {
                        "name": "Copper Rivers Trading renamed",
                        "country": "US",
                    },
                }
            ],
    ...
    with SessionLocal() as db:
        objects = db.scalars(
            select(SourceObject).where(
                SourceObject.local_id.in_(["negative-a", "negative-b"])
            )
        ).all()
